...
def images_label(path):
    ...
    for class_name in labels_name:
        current_id = class_id[:]
        index += 1
        current_id[index] += 1 #Setting to 1 the right position corresponding to the class of the boat
        img_list = os.listdir(path + '/' + class_name)
        for img in img_list:
            filename_list.append(path + '/' + class_name + '/' + img) #Contains images name
            label_list.append(current_id) #Each label matches to the same position of the above list 
    return filename_list, label_list

'''
This function is used to create two lists, one containing image names,
the other contains the respective
'''
def parsing_groundtruth(path):
	boat_label = dict()
	labels_name = iterate_dir('/home/matteo/Scrivania/argos_dataset/sc5') #Taking boat names to build labels
	#print(labels_name)
	i = 0
...
def decode_images(train_images, train_labels):
	...
	return resized_decoded_img, train_labels
...
filename_list, label_list = images_label('/home/matteo/Scrivania/argos_dataset/sc5')
training_dataset = create_dataset(filename_list, label_list)

testnames_list, testlabels_list = parsing_groundtruth('/home/matteo/Scrivania/argos_dataset/sc5-2013-Mar-Apr-Test-20130412')
test_dataset = create_dataset(testnames_list, testlabels_list)

summary_writer = tf.summary.FileWriter('/home/matteo/Scrivania/hw2_argos_summary', sess.graph)
...
batched_dataset = dataset_repeated.batch(100)
...
checkpoint_file = os.path.join('/home/matteo/Scrivania/hw2_argos_summary', 'model.ckpt')
